...
def load_dataset(dataset, normalize=False):
    ...
    if dataset == 'covtype':
        ...
        # Convert to binary classification for simplicity: class 2 and above -> class 1
        y = (y > 1).astype(int)
...
if __name__ == '__main__':
    ...
    num_classes = 1 if num_classes == 2 else num_classes  # Handle binary and multi-class

    y_train_one_hot = np.eye(num_classes)[y_train] if num_classes > 1 else y_train.reshape(-1, 1)
    y_val_one_hot = np.eye(num_classes)[y_val] if num_classes > 1 else y_val.reshape(-1, 1)
    y_test_one_hot = np.eye(num_classes)[y_test] if num_classes > 1 else y_test.reshape(-1, 1)
